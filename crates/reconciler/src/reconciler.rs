...
impl Connector for KubernetesConnector {
    ...
    async fn plan(
        ...
        if let Some(borrowed_environment) = desired.borrowed_environment() {
            if observed.state.proposal.is_some() {
                return proposal(
                    ExecutablePlan::CancelProposal,
                    desired,
                    "cancel review before borrowing stable state",
                );
            }
            ...
            world.record_cache_telemetry();
            let commit = match self.engine.applied_revision(borrowed_environment).await {
                Ok(commit) => commit,
                Err(error) => return engine_plan_failure(error),
            };
    ...
    async fn retire(
        ...
        if desired.borrowed_environment().is_none()
            && let Err(error) = self.engine.remove(&desired.environment).await
        {
            return RetireOutcome::Blocked(error.diagnostics().to_vec());
        }
